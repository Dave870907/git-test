...
def getContent():
    ...
    for url in url_list:
        ...
        sql = "insert into resultGossip (source,URL,content ) values ('PTT','"+ url +"','"+ content + "')" 
        sql2 = "DELETE from resultGossip where ID NOT IN (Select Max(ID) From resultGossip Group By URL)"    #刪除重複的資料      
      
        cursor.execute(sql)
        cursor.execute(sql2)
        connection.commit()  
def pttGossip(pagenum,keyword):
    ...
    for i in range(1,pagenum):  #range決定跑幾頁
        ...
        url ='https://www.ptt.cc/bbs/Gossiping/search?page='+str(i)+'&q='+ keyword
